...
export default function FormulaireOCPInteractive({
  ...
  /** Assistance IA : suggestions sections A/B/D/E (ne signe pas) */
  ...
  return (
    <Box sx={{ pb: 4 }}>
          ...
          <Box sx={{ display: 'flex', gap: 1 }}>
            ...
            {signMode !== 'ceee' && !fieldsLocked && (
              <Button
                variant="outlined"
                color="secondary"
                startIcon={iaLoading ? <CircularProgress size={14} /> : <DescriptionIcon />}
                onClick={handleAnalyserIA}
                disabled={loading || iaLoading}
                size="small"
              >
                Analyser description (IA)
              </Button>
            )}
      ...
      {(iaRapport || iaAlertes.length > 0) && (
        <Alert severity={iaAlertes.length ? 'warning' : 'success'} sx={{ mb: 1 }} onClose={() => { setIaRapport(null); setIaAlertes([]); }}>
          {iaRapport}
          {iaAlertes.length > 0 && (
          ...
          <Grid container sx={{ borderBottom: '1px solid #000', p: 0.75, alignItems: 'center' }}>
            ...
            <Grid size={4}>
              ...
              {readOnly ? (
                ...
                >
                  <option value="">Sélectionner service intervenant (E)...</option>
                  {servicesList.map((s) => (
                    <option key={s.id} value={s.nomService}>
                      {s.nomService}
                    </option>
          ...
          <Grid container sx={{ textAlign: 'center', fontSize: 11, fontWeight: 700 }}>
            <Grid size={4} sx={{ p: 1, borderRight: '1px solid #000' }}>
              ...
              {formData.valCeep ? (
                ...
                <Button size="small" sx={{ fontSize: 9, mt: 0.5 }} onClick={() => handleOpenSignature('valCeep')} disabled={fieldsLocked}>
                  Signer CEEP
                </Button>
              )}
            </Grid>
            <Grid size={4} sx={{ p: 1, borderRight: '1px solid #000' }}>
              ...
              {formData.valCeee ? (
                ...
                <Button size="small" sx={{ fontSize: 9, mt: 0.5 }} onClick={() => handleOpenSignature('valCeee')} disabled={fieldsLocked}>
                  Signer CEEE
                </Button>
              )}
            </Grid>
            <Grid size={4} sx={{ p: 1 }}>
              ...
              {formData.valSt ? (
                ...
                <Button size="small" sx={{ fontSize: 9, mt: 0.5 }} onClick={() => handleOpenSignature('valSt')} disabled={fieldsLocked}>
