...
/**
 * Bridge CrewAI multi-agents (Risques → HSE → Contrôle → Rapport) via FastAPI.
 */
...
public class CrewAIProvider implements IAProvider {
    ...
    public AnalyseIA analyserPermis(FichierJoint fichier, Permis permis) {
        throw new UnsupportedOperationException("Configurer ocp.ai.fastapi-url pour CrewAI.");
    }

    @Override
    public AnalyseInterventionIAResponse analyserIntervention(String description) {
        if (fastapiUrl == null || fastapiUrl.isBlank()) {
            throw new UnsupportedOperationException("CrewAI : définir ocp.ai.fastapi-url");
        }
