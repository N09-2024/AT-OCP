...
/**
 * Bridge LangChain direct via microservice Python FastAPI.
 */
...
public class LangChainProvider implements IAProvider {
    ...
    public AnalyseIA analyserPermis(FichierJoint fichier, Permis permis) {
        throw new UnsupportedOperationException("LangChain : analyse de fichier non supportée.");
    }

    @Override
    public AnalyzeAtResponse analyzeAt(AnalyzeAtRequest request) {
        if (fastapiUrl == null || fastapiUrl.isBlank()) {
            throw new IllegalStateException("LangChain : ocp.ai.fastapi-url non configuré.");
        }
    ...
    public AnalyseInterventionIAResponse analyserIntervention(String description) {
        if (fastapiUrl == null || fastapiUrl.isBlank()) {
            throw new IllegalStateException("LangChain : définir ocp.ai.fastapi-url");
        }
        return restTemplate.postForObject(
