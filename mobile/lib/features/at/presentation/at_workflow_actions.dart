...
///
/// RÈGLES PDF (rapport d'analyse §M) :
///   Bouton piloté par le DTO exportPdfAutorise / exportPdfMotifsRefus (calcul
///   serveur : statut ≥ soumise ET permis conformes) — aucune règle dupliquée.
///
...
class AtWorkflowActions extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // ── 3. Condition PDF — pilotée par le DTO (rapport d'analyse §M) ────────
    // exportPdfAutorise / exportPdfMotifsRefus sont calculés côté serveur
    // (calculerMotifsRefusExportPdf : statut ≥ soumise ET permis conformes).
    // Le mobile affiche/masque le bouton sur cette base, SANS dupliquer la règle.
    final bool pdfAutorise = at.exportPdfAutorise == true;

    final bool isWorkflowParticipant = roles.isWorkflowParticipant;

    final bool showPdf = pdfAutorise && isWorkflowParticipant;
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // Dialogue Démarrage — Readiness Check §8.3
  // ─────────────────────────────────────────────────────────────────────────

  Future<void> _dialogueDemarrage(BuildContext context, WidgetRef ref) async {
    bool habilitationsVerifiees = false;
    bool episEnPlace = false;
    bool consignationValidee = false;
    bool balisageConforme = false;
        ...
        builder: (ctx, setModalState) => AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                const Text(
                  'Point de contrôle Readiness Check préalable au démarrage :',
                  style: TextStyle(fontWeight: FontWeight.w700, fontSize: 13, color: OcpColors.ink),
                ),
                const SizedBox(height: 8),
                CheckboxListTile(
                  dense: true,
                  title: const Text('Intervenants habilités et autorisés sur site',
                      style: TextStyle(fontSize: 12)),
                  value: habilitationsVerifiees,
                  onChanged: (v) => setModalState(() => habilitationsVerifiees = v ?? false),
                ),
                CheckboxListTile(
                  dense: true,
                  title: const Text('EPI obligatoires portés et vérifiés', style: TextStyle(fontSize: 12)),
                  value: episEnPlace,
                  onChanged: (v) => setModalState(() => episEnPlace = v ?? false),
                ),
                CheckboxListTile(
                  dense: true,
                  title: const Text('Consignations et mesures de sécurité en place',
                      style: TextStyle(fontSize: 12)),
                  value: consignationValidee,
                  onChanged: (v) => setModalState(() => consignationValidee = v ?? false),
                ),
                CheckboxListTile(
                  dense: true,
                  title: const Text('Zone balisée et périmètre sécurisé', style: TextStyle(fontSize: 12)),
                  value: balisageConforme,
                  onChanged: (v) => setModalState(() => balisageConforme = v ?? false),
                ),
              ],
            ),
          ),
          actions: [
            ...
            FilledButton(
              ...
              onPressed: (habilitationsVerifiees && episEnPlace && consignationValidee && balisageConforme)
                  ? () async {
                      ...
                      await _executer(
                        ...
                        'Intervention démarrée avec succès. Statut : INTERVENTION EN COURS.',
                      );
                    }
                  : null,
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // Dialogue Fin des Travaux — Checklist §8.3
  // ─────────────────────────────────────────────────────────────────────────
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // Dialogue Reconduction (CEEE → HMEP)
  // ─────────────────────────────────────────────────────────────────────────
  ...
  Future<void> _reconduire(BuildContext context, WidgetRef ref) async {
    ...
    if (conf == true && context.mounted) {
      await _executer(
        context,
        ref,
        (api) => api.reconduire(at.id),
        'Demande de reconduction transmise au HMEP. Vous serez notifié de la décision.',
      );
    }
  }
