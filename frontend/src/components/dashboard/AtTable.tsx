...
const STATUT_CONFIG: Record<string, { label: string; bg: string; color: string }> = {
  ...
  SOUMISE: { label: 'Soumise', bg: '#DBEAFE', color: '#3B82F6' },
  ...
  RENOUVELEE: { label: 'Renouvelée', bg: '#EDE9FE', color: '#8B5CF6' },
  CLOTUREE: { label: 'Clôturée', bg: '#F3F4F6', color: '#4B5563' },
  ARCHIVEE: { label: 'Archivée', bg: '#F3F4F6', color: '#6B7280' },
  ANNULEE: { label: 'Annulée', bg: '#FEE2E2', color: '#EF4444' }
};
    ...
    <Card sx={{ p: 0, overflow: 'hidden' }}>
      ...
      <TableContainer>
        <Table>
          ...
          <TableBody>
            {data.map((row) => {
              const config = STATUT_CONFIG[row.statut] || { label: row.statut, bg: '#F3F4F6', color: '#6B7280' };
              return (
