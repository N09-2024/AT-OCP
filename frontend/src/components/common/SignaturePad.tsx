...
export default function SignaturePad({
  ...
  const initCanvas = useCallback(() => {
    ...
    ctx.strokeStyle = '#0f172a'; // Dark slate color for clean ink
    ctx.lineWidth = 2.5;
    ctx.lineCap = 'round';
    ctx.lineJoin = 'round';
  }, []);
  ...
  return (
    ...
    >
      ...
      {/* User info recap */}
      <Box sx={{ p: 2, mb: 2, bgcolor: '#f8fafc', borderRadius: 2, border: '1px solid #f1f5f9' }}>
      ...
      {isSaved && currentDataUrl ? (
        ...
        <Box
          sx={{
            position: 'relative',
            border: '2px dashed',
            borderColor: isDrawing ? '#059669' : '#cbd5e1',
            borderRadius: 2,
            bgcolor: disabled ? '#f1f5f9' : '#fafafa',
      ...
      <Dialog open={previewOpen} onClose={() => setPreviewOpen(false)} maxWidth="xs" fullWidth>
        ...
        <DialogContent>
          <Box sx={{ p: 2, border: '1px solid #e2e8f0', borderRadius: 2, bgcolor: '#ffffff', textAlign: 'center', my: 1 }}>
            ...
            {currentDataUrl && (
              <Box
                ...
                sx={{ maxHeight: 120, maxWidth: '100%', border: '1px solid #cbd5e1', p: 1, borderRadius: 1 }}
              />
            )}
          </Box>
        </DialogContent>
