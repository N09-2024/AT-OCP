    ...
    <Box sx={{ p: 3, bgcolor: '#f8fafc', minHeight: '100vh' }}>
      ...
      <Box sx={{ mb: 4, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <Button
          ...
          sx={{ borderRadius: 2, textTransform: 'none', fontWeight: 600, px: 3, bgcolor: '#00875A', '&:hover': { bgcolor: '#047857' } }}
        >
          Nouvelle AT
        </Button>
      </Box>
      ...
      <Grid container spacing={2} sx={{ mb: 4 }}>
        {[
          { label: 'AUTORISATIONS EN COURS', value: kpis.autorisationsEnCours, icon: <ClipboardDocumentCheckIcon width={32} color="#10b981" />, link: '/autorisations' },
          { label: 'VISAS EN ATTENTE', value: kpis.visasEnAttente, icon: <UserGroupIcon width={32} color="#10b981" />, link: '/autorisations?filtre=SOUMISE' },
          { label: 'PERMIS ACTIFS', value: kpis.permisActifs, icon: <ShieldCheckIcon width={32} color="#10b981" />, link: '/permis' },
          { label: 'RÉCEPTIONS', value: kpis.receptionsEnAttente, icon: <ArchiveBoxIcon width={32} color="#10b981" />, link: '/receptions' },
          { label: 'ARCHIVES', value: kpis.totalArchives, icon: <DocumentTextIcon width={32} color="#10b981" />, link: '/archives' },
        ].map((kpi) => (
                  ...
                  {recentATs.length === 0 ? (
                    ...
                    recentATs.map((at: any) => (
                      <TableRow key={at.id} hover>
                        <TableCell sx={{ fontWeight: 700, color: '#00875A' }}>{at.id}</TableCell>
            ...
            <Grid size={{ xs: 12, sm: 6 }}>
              <Paper sx={{ p: 3, borderRadius: 3, height: '100%' }}>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                  {[
                    { text: 'AT-2026-1258 validée par Ahmed', time: 'Il y a 25 min', bg: '#ecfdf5', icon: <ClipboardDocumentCheckIcon width={22} color="#10b981" /> },
                    { text: 'Visa requis pour AT-2026-1255', time: 'Il y a 1 heure', bg: '#fffbeb', icon: <UserGroupIcon width={22} color="#f59e0b" /> },
                    { text: 'Permis PFE-2026-045 expire demain', time: 'Il y a 2 heures', bg: '#ecfdf5', icon: <ShieldCheckIcon width={22} color="#10b981" /> },
                  ].map((act, i) => (
                ...
                <Box key={idx} sx={{ display: 'flex', gap: 2, alignItems: 'flex-start' }}>
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', minWidth: 20 }}>
                    <Box sx={{
                      ...
                      bgcolor: step.active ? '#00875A' : '#e2e8f0',
                      border: step.active ? '2px solid #00875A' : '2px solid #cbd5e1',
                      flexShrink: 0,
                    }} />
                    {idx < WORKFLOW_STEPS.length - 1 && (
                      <Box sx={{ width: 2, flex: 1, minHeight: 28, bgcolor: step.active ? '#00875A' : '#e2e8f0' }} />
                    )}
                  </Box>
                  {/* Content */}
                  <Box sx={{ pb: 2 }}>
                    <Typography variant="body2" sx={{ fontWeight: 700, color: step.active ? '#00875A' : '#94a3b8' }}>
                      {step.label}
                    </Typography>
