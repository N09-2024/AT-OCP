...
export default function AutorisationFormPage() {
  ...
  const handleSubmitAT = async (data: any, signatureBlob?: Blob) => {
    // Contrôle IA non bloquant (CrewAI/Mock) — l'humain reste décisionnaire
    try {
      const ctrl = await iaApi.controlerDossier({
        description: data?.description,
        visiteFaite: !!visiteId,
      ...
      /* IA indisponible : on continue le parcours manuel */
    }
  ...
  /** CEEE : signer uniquement la case CEEE du même formulaire F-HSE-SEC-31-04 */
  const handleVisaCeee = async (_data: any, signatureBlob: Blob) => {
    if (!atId) {
      alert("AT introuvable");
      return;
    }
    setLoading(true);
    try {
      await visaApi.createAndSignVisa(atId, signatureBlob, 'Visa CEEE — signature exécutant', 2);
      setStatusMsg('Visa CEEE enregistré. L\'AT est visée côté exécutant.');
      alert('Visa CEEE enregistré avec succès.');
      navigate(`/autorisations/${atId}`);
    } catch (err: any) {
      console.error(err);
      alert(err.response?.data?.message || 'Erreur lors de l\'enregistrement du visa CEEE.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <Container maxWidth="lg" sx={{ py: 3 }}>
      ...
      {activeStep === 2 && (
        ...
        <Box>
          ...
          {modeViser && (
            <Alert severity="info" sx={{ mb: 2 }}>
              Mode CEEE — même formulaire F-HSE-SEC-31-04. Signez uniquement la case <strong>Visa CEEE</strong> (section G), puis cliquez sur « Enregistrer visa CEEE ».
            </Alert>
          )}
