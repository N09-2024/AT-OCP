...
export default function ValidationOCPPage() {
  ...
  const handleValider = async () => {
    ...
    setSubmitting(true);
    try {
      // 1. Signer le visa du Responsable OCP
      await visaApi.createAndSignVisa(id, signatureBlob, commentaire || 'Visa de validation Responsable OCP', 2);
    ...
    <Box sx={{ p: 3, maxWidth: 1100, mx: 'auto' }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        ...
        <Typography variant="h5" sx={{ fontWeight: 800, color: '#0f172a' }}>
          Évaluation & Validation Responsable OCP
        </Typography>
      </Box>
      ...
      {/* Signature Section for Responsable OCP */}
      <Paper sx={{ p: 4, mb: 4, borderRadius: 3, border: '2px solid #00875A' }}>
        <Typography variant="h6" sx={{ fontWeight: 800, color: '#00875A', mb: 2 }}>
          Visa du Responsable OCP (CEEP / CEEE)
        </Typography>
      ...
      <Dialog open={rejectOpen} onClose={() => setRejectOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle sx={{ fontWeight: 800, color: '#e11d48' }}>
